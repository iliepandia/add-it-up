...
>
> **Update (2026-09-20, builds 108–113):** the merge above was **extended to Fast-mode prizes**
> (a Fast set pays out another Fast prize, badge and all) and its set size dropped from four to
> **two**; the prize box also got a scroll gutter and dark-wood end bars. Still **no §17 item
> moved** — this is the same prize-box economy widened and made cheaper to play, so the paragraph
> above now reads: the prize box is where the richest systems live, *both* modes play *both* of
> them, and none of them run on recall. Making a merge cost two duplicates instead of four raises
> how often that loop pays out relative to the maths, which sharpens 17.1's complaint rather than
> easing it.
